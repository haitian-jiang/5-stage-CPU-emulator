...
void load_use_stall(){
    ...
    if(em.inst.a != NOP && em.load_mem && (em.rt == de.rs || em.rt == de.rt)) {
		fd.inst = de.inst;
		fd.pc = de.pc;
		de.inst = MD_NOP_INST;
		de.opcode = 0;
...
void forward(){
    ...
    bool_t hazard_in_mw = (mw.inst.a != NOP) && (mw_rt_hazard || mw_rd_hazard);
  	...
  	if (hazard_in_em){
  		if ((em.write_rt && em.rt == de.rs) || (em.write_rd && em.rd == de.rs)){
  			de.rs_val = em.ALUout;
  		}else if ((em.write_rt && em.rt == de.rt) || (em.write_rd && em.rd == de.rt)) {
      		de.rt_val = em.ALUout;
    	}
	}else if(hazard_in_mw) {
    	if ((mw.write_rt && mw.rt == de.rs) || (mw.write_rd && mw.rd == de.rs)) {
      		de.rs_val = mw.load_mem?mw.from_mem:mw.ALUout;
    	}else if ((mw.write_rt && mw.rt == de.rt) || (mw.write_rd && mw.rd == de.rt)) {
    ...
    switch (de.opcode){
        ...
        case BNE:
            if (de.rs_val != de.rt_val){
                em.go_branch = 1;
                em.branch_addr = em.pc + sizeof(md_inst_t) + (OFS << 2);
            }
            break;
    }
}
